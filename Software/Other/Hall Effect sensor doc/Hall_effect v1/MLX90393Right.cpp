//
// MLX90393Right.cpp : arduino driver for MLX90393Right magnetometer
//
...
MLX90393Right()
{
  ...
  // gain steps derived from datarsheet section 15.1.4 tables
...
void
MLX90393Right::
cache_set(uint8_t address, uint16_t datar){
  if (address < cache_t::SIZE){
    cache.reg[address] = datar;
...
uint8_t
MLX90393Right::
readRegister(uint8_t address, uint16_t& datar)
{
  _i2cPort->beginTransmission(I2C_address);
  ...
  datar = (uint16_t(b_h)<<8) | b_l;
  cache_set(address, datar);
  return status;
}

uint8_t
MLX90393Right::
writeRegister(uint8_t address, uint16_t datar)
{
  cache_invalidate(address);

  _i2cPort->beginTransmission(I2C_address);
  if (_i2cPort->write(CMD_WRITE_REGISTER) != 1){ return STATUS_ERROR; }
  if (_i2cPort->write((datar & 0xff00) >> 8) != 1){ return STATUS_ERROR; }
  if (_i2cPort->write(datar & 0x00ff) != 1){ return STATUS_ERROR; }
  if (_i2cPort->write((address & 0x3f)<<2) != 1){ return STATUS_ERROR; }
  if (_i2cPort->endTransmission()){ return STATUS_ERROR; }
  if (_i2cPort->requestFrom(I2C_address, uint8_t(1)) != 1){ return STATUS_ERROR; }
  if (!_i2cPort->available()){ return STATUS_ERROR; }
...
MLX90393Right::txyz
MLX90393Right::
convertRaw(MLX90393Right::txyzRaw raw)
{
  ...
  if (tcmp_en){
    datar.x = ( (raw.x - 32768.f) * xy_sens *
               gain_factor * (1 << res_x) );
  } else {
    switch(res_x){
    case 0:
    case 1:
      datar.x = int16_t(raw.x) * xy_sens * gain_factor * (1 << res_x);
      break;
    case 2:
      datar.x = ( (raw.x - 32768.f) * xy_sens *
                 gain_factor * (1 << res_x) );
  ...
  if (tcmp_en){
    datar.y = ( (raw.y - 32768.f) * xy_sens *
               gain_factor * (1 << res_y) );
  } else {
    switch(res_y){
    case 0:
    case 1:
      datar.y = int16_t(raw.y) * xy_sens * gain_factor * (1 << res_y);
      break;
    case 2:
      datar.y = ( (raw.y - 32768.f) * xy_sens *
                 gain_factor * (1 << res_y) );
  ...
  if (tcmp_en){
    datar.z = ( (raw.z - 32768.f) * z_sens *
               gain_factor * (1 << res_z) );
  } else {
    switch(res_z){
    case 0:
    case 1:
      datar.z = int16_t(raw.z) * z_sens * gain_factor * (1 << res_z);
      break;
    case 2:
      datar.z = ( (raw.z - 32768.f) * z_sens *
                 gain_factor * (1 << res_z) );
      break;
    case 3:
      datar.z = ( (raw.z - 16384.f) * z_sens *
                 gain_factor * (1 << res_z) );
      break;
    }
  }

  datar.t = 25 + (raw.t - 46244.f)/45.2f;
  return datar;
}
...
convDelayMillis() {
  ...
  return
    (DRDY_pin >= 0)? 0 /* no delay if drdy pin present */ :
                     // estimate conversion time from datarsheet equations
...
uint8_t
MLX90393Right::
readdatar(MLX90393Right::txyz& datar)
{
