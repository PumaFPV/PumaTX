/*TO-DO
 * InPut Chan values to ppm (ppm is working but has no input data)
 * Make S-Bus work
...
//--------------------------------------------------Min--Max--Value--------------------------------------------------
int eepromaddrminthrottle = 1;    //EEPROM ADDRESS
int eepromaddrmaxthrottle = 2;
int eepromaddrminyaw = 3;
int eepromaddrmaxyaw = 4;
int eepromaddrminpitch = 5;
int eepromaddrmaxpitch = 6;
int eepromaddrminroll = 7;
int eepromaddrmaxroll = 8;
int eepromvalminthrottle, eepromvalmaxthrottle, eepromvalminyaw, eepromvalmaxyaw, eepromvalminpitch, eepromvalmaxpitch, eepromvalminroll, eepromvalmaxroll = 0;
...
//--------------------------------------------------EEPROM--------------------------------------------------
    ...
    EEPROM.write(eepromaddrminthrottle, map(minthrottle, minhallsensorval, maxhallsensorval, 0, 255));
...
//--------------------------------------------------NAVIGATION--------------------------------------------------
//Simple method
if (readingmenuright == LOW && page < 5) { //menu right -> page+
  ++page;
  delay(100);
}
