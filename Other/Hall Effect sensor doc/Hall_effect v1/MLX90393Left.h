...
#ifndef MLX90393Left_H_INCLUDED
#define MLX90393Left_H_INCLUDED
...
public:
  ...
  uint8_t readMeasurement(uint8_t zyxt_flags, iabcRaw& iabc_result);
  uint8_t readRegister(uint8_t address, uint16_t& datal);
  uint8_t writeRegister(uint8_t address, uint16_t datal);
  uint8_t reset();
...
private:
  ...
  void cache_set(uint8_t address, uint16_t datal);
  uint8_t cache_fill();
...
#endif // #ifndef MLX90393Left_H_INCLUDED
