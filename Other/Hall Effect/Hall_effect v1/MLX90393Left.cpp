//
// MLX90393Left.cpp : arduino driver for MLX90393Left magnetometer
//
...
MLX90393Left()
{
  ...
  // gain steps derived from datallsheet section 15.1.4 tables
...
void
MLX90393Left::
cache_set(uint8_t address, uint16_t datal){
  if (address < cache_t::SIZE){
    cache.reg[address] = datal;
...
uint8_t
MLX90393Left::
readMeasurement(uint8_t zyxt_flags, iabcRaw& iabc_result)
{
  ...
  if (zyxt_flags & T_FLAG){
    iabc_result.t =  (uint16_t(buffer[i])<<8) | buffer[i+1];
    i += 2;
  } else {
    iabc_result.t = 0;
  }
  if (zyxt_flags & X_FLAG){
    iabc_result.x =  (uint16_t(buffer[i])<<8) | buffer[i+1];
    i += 2;
  } else {
    iabc_result.x = 0;
  }
  if (zyxt_flags & Y_FLAG){
    iabc_result.y =  (uint16_t(buffer[i])<<8) | buffer[i+1];
    i += 2;
  } else {
    iabc_result.y = 0;
  }
  if (zyxt_flags & Z_FLAG){
    iabc_result.z =  (uint16_t(buffer[i])<<8) | buffer[i+1];
    i += 2;
  } else {
    iabc_result.z = 0;
  }
...
uint8_t
MLX90393Left::
readRegister(uint8_t address, uint16_t& datal)
{
  _i2cPort->beginTransmission(I2C_address);
  ...
  datal = (uint16_t(b_h)<<8) | b_l;
  cache_set(address, datal);
  return status;
}

uint8_t
MLX90393Left::
writeRegister(uint8_t address, uint16_t datal)
{
  cache_invalidate(address);

  _i2cPort->beginTransmission(I2C_address);
  if (_i2cPort->write(CMD_WRITE_REGISTER) != 1){ return STATUS_ERROR; }
  if (_i2cPort->write((datal & 0xff00) >> 8) != 1){ return STATUS_ERROR; }
  if (_i2cPort->write(datal & 0x00ff) != 1){ return STATUS_ERROR; }
  if (_i2cPort->write((address & 0x3f)<<2) != 1){ return STATUS_ERROR; }
  if (_i2cPort->endTransmission()){ return STATUS_ERROR; }
  if (_i2cPort->requestFrom(I2C_address, uint8_t(1)) != 1){ return STATUS_ERROR; }
  if (!_i2cPort->available()){ return STATUS_ERROR; }
...
MLX90393Left::iabc
MLX90393Left::
convertRaw(MLX90393Left::iabcRaw raw)
{
  ...
  if (tcmp_en){
    datal.x = ( (raw.x - 32768.f) * xy_sens *
               gain_factor * (1 << res_x) );
  } else {
    switch(res_x){
    case 0:
    case 1:
      datal.x = int16_t(raw.x) * xy_sens * gain_factor * (1 << res_x);
      break;
    case 2:
      datal.x = ( (raw.x - 32768.f) * xy_sens *
                 gain_factor * (1 << res_x) );
  ...
  if (tcmp_en){
    datal.y = ( (raw.y - 32768.f) * xy_sens *
               gain_factor * (1 << res_y) );
  } else {
    switch(res_y){
    case 0:
    case 1:
      datal.y = int16_t(raw.y) * xy_sens * gain_factor * (1 << res_y);
      break;
    case 2:
      datal.y = ( (raw.y - 32768.f) * xy_sens *
                 gain_factor * (1 << res_y) );
  ...
  if (tcmp_en){
    datal.z = ( (raw.z - 32768.f) * z_sens *
               gain_factor * (1 << res_z) );
  } else {
    switch(res_z){
    case 0:
    case 1:
      datal.z = int16_t(raw.z) * z_sens * gain_factor * (1 << res_z);
      break;
    case 2:
      datal.z = ( (raw.z - 32768.f) * z_sens *
                 gain_factor * (1 << res_z) );
      break;
    case 3:
      datal.z = ( (raw.z - 16384.f) * z_sens *
                 gain_factor * (1 << res_z) );
      break;
    }
  }

  datal.t = 25 + (raw.t - 46244.f)/45.2f;
  return datal;
}
...
convDelayMillis() {
  ...
  return
    (DRDY_pin >= 0)? 0 /* no delay if drdy pin present */ :
                     // estimate conversion time from datalsheet equations
...
uint8_t
MLX90393Left::
readdatal(MLX90393Left::iabc& datal)
{
  ...
  iabcRaw raw_iabc;
  uint8_t status2 =
    readMeasurement(X_FLAG | Y_FLAG | Z_FLAG | T_FLAG, raw_iabc);
  datal = convertRaw(raw_iabc);
